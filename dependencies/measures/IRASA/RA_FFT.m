...
function [RA_FFT,Smixd, Sfrac, freq] = RA_FFT(sig,srate,nfft,hset,flag_filter)
...
Nfrac = nfft;
freq = srate/2*linspace(0,1,nfft); freq = freq(:);
...
% end
% Smixd = Smixd/Nsubset;
orig_fft = fft(sig,nfft);
%orig_fft(2:end,:) = orig_fft(2:end,:)*2;
...
for ih = 1:length(hset)
    ...
    %         Sh = Sh + tmp(1:Nfrac,:);
    %     end
    sig_h = myresample(sig,n,d);
    ph = fft(sig_h,nfft);
    %ph(2:end,:) = ph(2:end,:)*2; %why do we multiply by 2?
    tmp = (abs(ph)).^2;
    Sh = tmp(1:Nfrac,:);
    %Sh = Sh / Nsubset;
    ...
    %     end
    if (flag_filter == 1)
        sig_1h = myresample(sig_filtered,d,n);
    else
        sig_1h = myresample(sig,d,n);
    end
    p1h = fft(sig,nfft);
    %p1h(2:end,:) = p1h(2:end,:)*2;
    tmp = (abs(p1h)).^2;
...
RA_FFT = Sub_complex(orig_fft,sqrt(Sfrac));
%RA_FFT = RA_FFT(2:end);
end
