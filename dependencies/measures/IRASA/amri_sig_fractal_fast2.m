...
function spec = amri_sig_fractal_fast(sig,srate,varargin)
...
x1 = zeros(Ndata,size(sig,2),Nsubset); x1u = x1;
for k = 0 : Nsubset-1
    i0 = L*k + 1;
    x1(:,:,k+1) = sig(i0:i0+Ndata-1,:);
    if (flag_filter==1)
        x1u(:,:,k+1) = sig_filtered(i0:1:i0+Ndata-1,:);
    else
        x1u(:,:,k+1) = sig(i0:1:i0+Ndata-1,:);
    end
end
...
for ih = 1:length(hset)
    ...
    %for k = 0 : Nsubset-1
        xh = resample(x1, n, d); 
        taperh = gettaper(size(xh)); % can precompute this - same every time
        ph = fft(xh.*taperh,nfft)/min(nfft,size(xh,1));
        %ph(2:end,:) = ph(2:end,:)*2;
        tmp = 4.*(abs(ph)).^2;
        tmp = reshape(tmp,[],size(x1,2),size(x1,3));
        Sh = sum(tmp(1:Nfrac,:,:),3)/Nsubset;
        %Sh = Sh + tmp(1:Nfrac,:);
   % end
